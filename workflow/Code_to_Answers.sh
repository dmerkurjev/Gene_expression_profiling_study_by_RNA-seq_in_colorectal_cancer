...
# q1: How many sequencing lanes were concatenated to form sample A375rep1?
...
# q2: What sample has lower library size (total read counts):SHSY5Yrep1 or HEK293rep1?
libsize <- colSums(counts)
ans_q2 <- unname(libsize["sm"])

# q3: How many genes have counts>20 in sample HEK293rep1 or counts>20 in sample HEK293rep2?
ans_q3 <- sum(counts[, "ym"] > 0)

# q4: How many genes are upregulated \uc0\u8805 2-fold (log2FC \u8805  1) in SHSY5Y vs. HEK293 with FDR < 0.01?
res_df <- as.data.frame(res)
ans_q4 <- sum(res_df$log2FoldChange >= 1 & res_df$padj < 0.01, na.rm = TRUE)

# q5: Which gene is ranked 3th by log2 fold change (most upregulated) in A375 vs. HEK293?
